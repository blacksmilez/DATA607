...
The group has used Azure database to store the tables, so all th egroup members could access it any time. As an alternative, you can store the tables on the local machine.
...
```
We have created necessary sql tables to store the info from R dataframes.
```{r create-tables,eval=False}
...
```{r load data into a database, eval=False}
dbListTables(conn)
...
```
The below tables will unite infor about every inspection and violation.
```{r}
...
Instead of the borough code (1,2,3, etc) and action code (01, 02, etc)we will use the borough names (Manhattan, Bronx, etc) nd action descriptions, the description of each borough code is stored in the tables tbl_boro, action is in the action_code table. \
```{r}
tbl_boro
action_code
```
